...
void main() {
  group('AccountService edge cases', () {
    test(
      'calcMoney returns a null ratio when receivable is effectively zero',
      () {
    ...
    test(
      'buildEffectiveRateMap keeps defaults and allows override-only device ids',
      () {
        ...
        expect(result, {1: 120.0, 8: 200.0});
      },
    );

    test(
      'calcRateInfo returns null minRate when no positive effective rate is used',
      () {
        const agg = ProjectAgg(
          projectKey: 'Alice||Yard A',
          contact: 'Alice',
    ...
    test(
      'calcMoney allows over-received amounts and returns negative remaining',
      () {
        const agg = ProjectAgg(
          projectKey: 'Over||Pay',
          contact: 'Over',
    ...
    test(
      ...
      () {
        ...
        final siteAId = ProjectId.legacyFromParts(
          contact: 'Alpha',
          site: 'Site A',
        );
        final siteBId = ProjectId.legacyFromParts(
          contact: 'Alpha',
          site: 'Site B',
        );
        expect(projects.keys.toSet(), {siteAId, siteBId});
        expect(projects[siteAId]!.projectKey, 'Alpha||Site A');
        expect(projects[siteAId]!.minYmd, 20260301);
        expect(projects[siteAId]!.deviceIds, [1, 2]);
        expect(projects[siteAId]!.hoursByDevice, {1: 2, 2: 3});
        expect(projects[siteAId]!.rentIncomeTotal, 100);
        expect(projects[siteBId]!.hoursByDevice, {3: 1});
      },
    );
    ...
    test('same contact and site can stay separate by projectId', () {
      ...
      expect(projects.keys.toSet(), {'project:first', 'project:second'});
      expect(projects['project:first']!.projectKey, 'Alpha||Same Site');
      expect(projects['project:first']!.hoursByDevice, {1: 2});
      expect(projects['project:second']!.hoursByDevice, {1: 3});
      ...
      expect(firstMoney.receivable, 300);
      expect(firstMoney.received, 0);
    });
