...
///   quantity_scaled 仍 nullable；A4-7 起 income REAL 已删除。
...
void main() {
  ...
  test(
    'A: fresh schema enforces income_fen NOT NULL and drops income REAL',
    () async {
  ...
  test('B: drifted v33 schema is rebuilt by onOpen ensure without losing rows '
      'or calculation history children', () async {
  ...
  test('C: AUTOINCREMENT high-water mark survives the rebuild (pit A)', () async {
    final db = await databaseFactoryFfi.openDatabase(
      dbPath,
      options: OpenDatabaseOptions(
        version: 33,
  ...
  test(
    'E: FK RESTRICT still rejects orphan project ids after rebuild',
    () async {
      final db = await databaseFactoryFfi.openDatabase(
        dbPath,
        options: OpenDatabaseOptions(
