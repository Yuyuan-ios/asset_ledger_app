...
///   A4-6 后 amount / merge_batch_total_amount REAL 已删除；id AUTOINCREMENT；
///   idx 与 FK RESTRICT 在 DDL；manual 收款不提供 merge fen 仍可插入。
...
void main() {
  ...
  test('fresh schema: amount_fen NOT NULL, merge fen nullable, AUTOINCREMENT + '
    ...
    try {
      ...
      expect(
        (await db.query(
          'account_payments',
        )).single['merge_batch_total_amount_fen'],
        isNull,
      );
    } finally {
      await db.close();
  ...
  test('legacy (no amount_fen column) -> v31: NOT NULL, COALESCE backfilled, '
    ...
    try {
      ...
      expect(rows.map((row) => row.containsKey('amount')).toSet(), {false});
      expect(
        rows.map((row) => row.containsKey('merge_batch_total_amount')).toSet(),
        {false},
      );
      expect(rows.map((row) => (row['amount_fen'] as num?)?.toInt()).toList(), [
        12345,
        10,
        20000,
      ]);
  ...
  // ---------------------------------------------------------------------------
  test(
    '坑A: AUTOINCREMENT high-water mark survives rebuild (old_seq > MAX(id))',
    () async {
      ...
      try {
        ...
        // 1) 高水位未倒退。
        expect(
          await _seq(db, 'account_payments'),
          greaterThanOrEqualTo(oldSeq),
        );
        ...
        final ids = (await db.query(
          'account_payments',
          columns: ['id'],
        )).map((row) => (row['id'] as num).toInt()).toList();
  ...
  // ---------------------------------------------------------------------------
  test(
    'ensure idempotent: first rebuild then no-op, data + AUTOINCREMENT intact',
    () async {
      final db = await _openLegacyNullableAtCurrentVersion(dbPath);
      try {
        await _insertProject(db, id: 'project:a');
