...
/// Track A / A2b：project_device_rates.rate_fen 提升为 NOT NULL。
void main() {
  ...
  setUp(() async {
    tmpDir = await Directory.systemTemp.createTemp('project_rate_fen_nn_');
  ...
  test('fresh schema enforces rate_fen NOT NULL and keeps FK/index', () async {
  ...
  test(
    'legacy v38 nullable rate_fen is rebuilt, backfilled, and keeps rows',
    () async {
      final legacy = await databaseFactoryFfi.openDatabase(
        ...
        options: OpenDatabaseOptions(
          ...
          onConfigure: _enableForeignKeys,
          onCreate: (db, _) async {
            await _createV38ProjectDeviceRates(db);
      ...
      expect(
        await _isNotNull(legacy, 'project_device_rates', 'rate_fen'),
        isFalse,
      );
      await legacy.close();
      ...
      final upgraded = await databaseFactoryFfi.openDatabase(
        ...
        options: OpenDatabaseOptions(
          ...
          onOpen: DbMigrations.ensureProjectDeviceRateFenNotNull,
        ),
      );
      try {
        expect(
          await _isNotNull(upgraded, 'project_device_rates', 'rate_fen'),
          isTrue,
        );
        ...
        expect(rows[1]['rate_fen'], 1, reason: '既有非 NULL fen 不应被重建覆盖');

        final rate = ProjectDeviceRate.fromMap(rows.first);
        expect(rate.rateFen, 39999);
      } finally {
    ...
    () async {
      final db = await databaseFactoryFfi.openDatabase(
        ...
        options: OpenDatabaseOptions(
          ...
          onCreate: (db, _) async {
            ...
            await db.insert(
              'project_device_rates',
              _rateRow(projectId: 'project:fk', rate: 380.0),
            );
          },
        ),
      );
      try {
        ...
        await expectLater(
          db.insert(
            'project_device_rates',
            _rateRow(projectId: 'project:missing', rate: 1.0, rateFen: 100),
          ),
          throwsA(isA<DatabaseException>()),
        );
        await expectLater(
...
Future<void> _createV38ProjectDeviceRates(DatabaseExecutor db) async {
  await db.execute('''
