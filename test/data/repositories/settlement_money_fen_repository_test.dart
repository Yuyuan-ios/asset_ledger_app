...
/// - amount REAL 仅作为 legacy / display 兼容字段。
///
/// 本测试用"故意写入 amount REAL 与 amount_fen 不一致的行"来检测权威路径：
/// 如果某个 SUM 仍然走 REAL，结果会被脏 REAL 数据带偏；走 fen 才能稳定通过。
void main() {
  ...
  group('ProjectWriteOffRepository sum 权威走 amount_fen', () {
    test('amount REAL 与 amount_fen 故意不一致时，sum 必须以 fen 为准', () async {
      final db = await AppDatabase.database;
      await _seedProject(db, projectId: 'project:alpha');

      // 写入两行：amount REAL 故意写成 999.99（脏数据），amount_fen 为权威 12345 / 6789。
      ...
      expect(
        ...
        (12345 + 6789) / 100.0,
        reason: '权威汇总应以 SUM(amount_fen) 为准，不受脏 REAL amount 影响',
      );
      expect(
        await repository.sumByProjectIds(['project:alpha']),
      ...
      expect(
        await repository.sumFenByProjectIds(['project:alpha']),
    ...
    test('0.1 + 0.2 的 REAL 累加误差不会污染 fen 汇总', () async {
      ...
      final repository = SqfliteProjectWriteOffRepository();
      expect(await repository.sumFenByProjectId('project:alpha'), 30,
  ...
  group('LocalProjectSettlementRepository settle / 撤销 以 fen 为权威', () {
    test(
      'settle 判断结清以 amount_fen 为准（REAL amount 被脏数据污染时仍然结清）',
      () async {
        final db = await AppDatabase.database;
        await _seedProject(db, projectId: 'project:alpha');
    ...
    test(
      'settle 抛错"已结清"以 fen 为准：amount_fen 已覆盖待收时，应拒绝再次结清',
      () async {
        final db = await AppDatabase.database;
        await _seedProject(db, projectId: 'project:alpha');
        ...
        await expectLater(
          ...
          reason: 'remaining = 100 - 100 = 0 在 fen 下应判已结清，'
    ...
    test(
      'settle 判断超出待收以 fen 为准：REAL 看似还差很多，fen 已接近覆盖',
      () async {
        final db = await AppDatabase.database;
        await _seedProject(db, projectId: 'project:alpha');
...
/// 直接落库一行 write_off：故意把 amount REAL 与 amount_fen 脱钩，
/// 用来探测权威 SUM 走的是哪一列。
