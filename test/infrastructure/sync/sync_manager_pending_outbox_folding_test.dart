...
void main() {
  ...
  group('sync_manager_folds_update_before_delete', () {
    test('update followed by delete for the same entity: update is folded; '
        'delete pushes normally and is acked', () async {
      final db = await AppDatabase.database;
      await _insertOutbox(
        db,
        mark: 'U1',
      ...
      expect(client.sentMarks, <String>[
        'D',
    ...
    test(
      'folded update stays removed when delete send fails and retries later',
      () async {
        final db = await AppDatabase.database;
        await _insertOutbox(
          db,
        ...
        await _insertMeta(
          db,
          entityType: 'account_payment',
          localId: 'retry-delete',
          status: 'pendingDelete',
        );

        final firstClient = _ProgrammableClient(failMarks: {'D'});
        final firstResult = await managerWith(firstClient).pushPending();

        expect(firstClient.sentMarks, <String>['D']);
        expect(firstResult.folded, 1);
        expect(firstResult.pushed, 0);
        expect(firstResult.failed, 1);
        expect(firstResult.invalid, 0);
        expect(
          await _rowByMark(db, 'U'),
          isNull,
          reason: 'folded update must stay deleted after delete failure',
        );
        final failedDelete = await _rowByMark(db, 'D');
        expect(failedDelete, isNotNull);
        expect(failedDelete!['status'], 'pending');
        expect(failedDelete['retry_count'], 1);
        expect(failedDelete['last_error'].toString(), contains('server_error'));
        expect(
          failedDelete['next_retry_at'],
          fixedNow.add(const Duration(seconds: 60)).toIso8601String(),
        );
        expect(
          await _metaStatus(db, 'account_payment', 'retry-delete'),
          'pendingDelete',
          reason: 'delete meta lifecycle remains deferred in R5.23',
        );

        await _forceDue(db, 'D');

        final retryClient = _ProgrammableClient.alwaysSuccess();
        final retryResult = await managerWith(retryClient).pushPending();

        expect(retryClient.sentMarks, <String>['D']);
        expect(retryResult.folded, 0);
        expect(retryResult.pushed, 1);
        expect(retryResult.failed, 0);
        expect(retryResult.invalid, 0);
        expect(
          await _rowByMark(db, 'U'),
          isNull,
          reason: 'folded update is not re-created for retry',
        );
        expect(await _rowByMark(db, 'D'), isNull);
        expect(
          await _metaStatus(db, 'account_payment', 'retry-delete'),
          'pendingDelete',
          reason: 'successful delete ack keeps current deferred meta semantics',
        );
      },
    );
  });
  ...
  group('sync_manager_folding_with_transaction_group', () {
    test('ungrouped create + ungrouped delete fold cleanly (group invariant '
        'untouched)', () async {
      final db = await AppDatabase.database;
      await _insertOutbox(
        db,
    ...
    test('two single-row groups (create in group A, delete in group B) fold '
        'safely: each group disappears whole, surviving groups push without '
        'invalid-metadata', () async {
      final db = await AppDatabase.database;
      await _insertOutbox(
        db,
        mark: 'CA',
    ...
    test('mixed group (settlement cluster): folding aborts for the entity '
        'whose create+delete would gap a group; the cluster pushes intact, no '
        'invalid_metadata', () async {
      ...
      expect(client.sentMarks, <String>['S-PAY', 'S-WO', 'S-PRJ', 'LATER-DEL']);
      expect(result.folded, 0);
      expect(result.pushed, 4);
      expect(result.invalid, 0);
    });
    ...
    test(
      'partial group in snapshot does not fold same entity grouped row',
      () async {
        // Defensive legacy/bad-data case: one row from txn-partial is still in
        // a future backoff window, so the due snapshot sees only part of the
        // transaction group. Folding must not delete the visible same-entity
        // grouped row and manufacture a sequence gap.
        final db = await AppDatabase.database;
        await _insertOutbox(
          db,
          mark: 'PAY-C',
          entityType: 'account_payment',
          entityId: 'partial-pay',
          operation: 'create',
          groupId: 'txn-partial',
          seq: 1,
          createdAt: '2026-06-01T00:00:01.000Z',
        ...
        final client = _ProgrammableClient(failMarks: {'ANCHOR'});
        final result = await managerWith(client).pushPending();

        expect(client.sentMarks, <String>['PAY-C', 'ANCHOR', 'PAY-D']);
        expect(result.folded, 0);
        expect(result.pushed, 2, reason: 'PAY-C and PAY-D are acked');
        expect(result.failed, 1, reason: 'ANCHOR uses ordinary backoff');
        expect(result.invalid, 0);
        expect(
          await _rowByMark(db, 'PAY-C'),
          isNull,
          reason: 'PAY-C was sent and acked, not deleteSuperseded',
        );
        expect(await _rowByMark(db, 'PAY-D'), isNull);
        final anchor = await _rowByMark(db, 'ANCHOR');
        expect(anchor, isNotNull);
        expect(anchor!['status'], 'pending');
        expect(anchor['retry_count'], 1);
        expect(anchor['last_error'].toString(), contains('server_error'));
        expect(
          anchor['last_error'].toString(),
          isNot(contains('invalid_metadata')),
        );
        expect(
          anchor['next_retry_at'],
          fixedNow.add(const Duration(seconds: 60)).toIso8601String(),
        );
        final future = await _rowByMark(db, 'FUTURE');
        expect(future, isNotNull);
        expect(future!['status'], 'pending');
        expect(future['retry_count'], 0);
        expect(future['last_error'], isNull);
        expect(future['next_retry_at'], '2099-01-01T00:00:00.000Z');

        final secondClient = _ProgrammableClient.alwaysSuccess();
        final secondResult = await managerWith(secondClient).pushPending();
        expect(secondClient.sentMarks, isEmpty);
        expect(secondResult.folded, 0);
        expect(secondResult.invalid, 0);
        expect(secondResult.pushed, 0);
  ...
  group('sync_manager_folding_entity_sync_meta_breadcrumb', () {
    test('create+delete fold clears the outbox so meta-driven re-push cannot '
        'happen; meta row is intentionally untouched (deferred deleted-entity '
        'meta lifecycle)', () async {
      final db = await AppDatabase.database;
      await _insertOutbox(
        db,
        mark: 'C',
...
/// Force a row's next_retry_at into the past so the next push retries it.
Future<void> _forceDue(Database db, String mark) async {
  await db.rawUpdate(
