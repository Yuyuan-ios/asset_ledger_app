...
void main() {
  ...
  test(
    'pushPending throws SyncPushBlockedException when push gate is set; '
    'does not list pending rows or call CloudApiClient',
    () async {
      ...
      expect(spyClient.sendCallCount, 0, reason: 'CloudApiClient must not be called when gated');
      ...
      // （EmptyCloudApiClient 行为：返回 204、isSuccess=false，不计入 pushed），
      // 关键断言是「不再被 gate 拦截」。
  ...
  test('mark/clear push gate round-trip through the repository helper', () async {
    const gateRepo = LocalSyncStateRepository();
    await AppDatabase.inTransaction<void>(
      (txn) => gateRepo.markPushGateRestorePendingWithExecutor(txn),
    );
