...
void main() {
  ...
  group('evaluate — 单项目结清影响判断', () {
    test('已结清 + 应收上调后 remainingFen > 0 → shouldRevokeSettlement = true',
        () async {
    ...
    test('已结清 + 收款+核销仍覆盖应收 → shouldRevokeSettlement = false',
        () async {
      final db = await AppDatabase.database;
      await _seedProject(
        db,
    ...
    test('0 元空项目：receivable=0 / received=0 / writeOff=0 → 不能视为可结清',
        () async {
      final db = await AppDatabase.database;
    ...
    test('0 元空项目但状态为 settled：被视为需要撤销（§5 兜底）',
        () async {
      final db = await AppDatabase.database;
      await _seedProject(
        db,
    ...
    test('amount REAL 与 amount_fen 故意不一致时，判断必须以 fen 为准', () async {
      ...
      final snapshot = decision.snapshots.single;
      expect(snapshot.receivedFen, 10000,
          reason: '权威已收必须从 SUM(amount_fen) 读出 10000，'
              '不能被脏 REAL=0.01 污染');
      expect(snapshot.coversReceivable, isTrue);
      expect(snapshot.shouldRevokeSettlement, isFalse);
    });

    test('差 1 fen 边界：fen=9999 vs receivable=10000 → 未覆盖，需撤销结清',
        () async {
      final db = await AppDatabase.database;
      await _seedProject(
        db,
      ...
      expect(snapshot.coversReceivable, isFalse,
          reason: '差 1 fen 不能算覆盖');
      expect(snapshot.shouldRevokeSettlement, isTrue);
    });

    test('刚好覆盖：fen=10000 = receivable=10000 → coversReceivable=true，不撤销',
        () async {
      final db = await AppDatabase.database;
      await _seedProject(
        db,
  ...
  group('evaluate — 多项目批量评估，可被 delete/edit/dissolve 共用', () {
    test('一批项目里只有需要撤销的子集出现在 revocationsNeeded', () async {
      ...
      expect(
        decision.revocationsNeeded.map((s) => s.projectId).toSet(),
        {'p:revoke'},
      );
      expect(decision.anyRevocationNeeded, isTrue);
    });
  ...
  group('applyRevocations — 仅撤销 status，不删业务记录', () {
    test('applyRevocations 只把 settled → active，不删 payments / write_offs',
        () async {
      final db = await AppDatabase.database;
      await _seedProject(
        db,
    ...
    test('evaluate + applyRevocations 可在同一事务内联用', () async {
      ...
      expect(
        (await db.query('projects',
                where: 'id = ?', whereArgs: ['project:alpha']))
            .single['status'],
