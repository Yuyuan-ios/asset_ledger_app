...
void main() {
  group('ComputeAccountSummaryUseCase edge cases', () {
    test(
      'returns empty summaries and a null total ratio when there is no receivable',
      () {
        ...
        expect(result.settlementRate, isNull);
        expect(result.deviceReceivables, isEmpty);
      },
    );

    test(
      'creates fallback device names for unknown ids and sorts them by name length',
      () {
        const useCase = ComputeAccountSummaryUseCase();
