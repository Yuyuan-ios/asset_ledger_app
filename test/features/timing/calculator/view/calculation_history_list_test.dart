...
void main() {
  testWidgets('shows existing and staged histories newest first', (
    ...
    expect(find.text('8 + 8.2 + 8.3 + 8.1 = 32.6 h'), findsOneWidget);
    expect(find.text('8 + 8.2 + 7.8 = 24.0 h'), findsOneWidget);

    final stagedTop = tester.getTopLeft(find.textContaining('[本次]')).dy;
    final existingTop = tester.getTopLeft(find.textContaining('[已保存]')).dy;
