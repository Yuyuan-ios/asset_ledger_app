...
/// Example:
///   dart run tools/gen_figma_tokens.dart assets/tokens/tokens.json lib/presentation/theme/app_tokens.g.dart
///
/// Supports:
...
void main(List<String> args) async {
  if (args.length < 2) {
    stderr.writeln('Usage: dart run tools/gen_figma_tokens.dart <tokens.json> <out_dart>');
    exit(64);
  }
  final inPath = args[0];
  ...
  bool isColorType(_TokenLeaf t) =>
      ...
      t.path.isNotEmpty && (t.path.first == 'color' || t.path.first == 'colors');

  bool isRadiusType(_TokenLeaf t) =>
      (t.type?.toLowerCase() == 'dimension' || t.type?.toLowerCase() == 'number') &&
      t.path.isNotEmpty &&
      (t.path.first == 'radius' || t.path.first == 'radii' || t.path.first == 'cornerRadius');
  ...
  bool isTextType(_TokenLeaf t) =>
      ...
      (t.path.isNotEmpty && (t.path.first == 'text' || t.path.first == 'typography'));
  ...
  for (final t in leaves) {
    if (isColorType(t)) {
      ...
      final name = _toCamel(_dropPrefix(t.path, ['radius', 'radii', 'cornerRadius']));
  ...
  for (final k in tKeys) {
    ...
    if (spec.fontWeight != null) buf.writeln('    fontWeight: FontWeight.w${spec.fontWeight},');
    if (spec.height != null) buf.writeln('    height: ${spec.height},');
    if (spec.colorName != null) buf.writeln('    color: AppColors.${spec.colorName},');
    buf.writeln('  );');
    buf.writeln();
  }
  buf.writeln('}');
...
String _toCamel(List<String> parts) {
  ...
  final cleaned = parts.map((p) => p.replaceAll(RegExp(r'[^a-zA-Z0-9]+'), ' ')).join(' ');
  final words = cleaned.trim().split(RegExp(r'\s+')).where((w) => w.isNotEmpty).toList();
...
String? _parseColorToDart(dynamic v) {
  ...
  if (rgba != null) {
    ...
    if (parts.length >= 3) {
      ...
      final hex = ((ai & 0xFF) << 24) | ((r & 0xFF) << 16) | ((g & 0xFF) << 8) | (b & 0xFF);
...
_TextStyleSpec? _parseTextStyle(dynamic v) {
  ...
  if (v is Map) {
    ...
    if (fwRaw is String) fw = int.tryParse(fwRaw.replaceAll(RegExp(r'[^0-9]'), ''));
    final lh = v['lineHeight'] ?? v['height'] ?? v['line_height'];
    double? height;
    if (lh is num) height = lh.toDouble();
    if (lh is String) height = double.tryParse(lh.replaceAll(RegExp(r'[^0-9\.\-]'), ''));
    final color = v['color'];
    String? colorName;
    if (color is String) {
      // Allow "colors.textPrimary" or "{color.textPrimary}"
      final c = color.replaceAll('{', '').replaceAll('}', '').replaceAll('colors.', '').replaceAll('color.', '');
      colorName = _toCamel(c.split('.'));
    }
    return _TextStyleSpec(fontSize: fs, fontWeight: fw, height: height, colorName: colorName);
  }
  return null;
}
