  ...
  static const List<LegalSectionContent> _sections = [
    ...
    LegalSectionContent(
      ...
      body:
          ...
          '上述业务数据在当前版本中主要存储在你的设备本地。为实现手机号验证码登录，你输入的手机号、验证码校验请求、登录状态及必要的服务端响应信息会发送至开发者配置的账号接口，并由阿里云号码认证服务提供短信验证码发送与校验能力。当前版本未接入广告 SDK、行为分析 SDK、第三方追踪服务或开发者侧业务数据云同步服务。',
    ),
    LegalSectionContent(
      title: '3. 数据来源与用途说明',
      body:
          ...
          '除手机号验证码登录、你主动通过系统能力发起评分、邮件联系或打开外部链接等行为外，开发者当前不会通过本应用主动接收你在应用内录入的业务数据。',
    ),
    LegalSectionContent(
      title: '4. 权限使用说明',
    ...
    LegalSectionContent(
      ...
      body:
          '当前版本下，你输入的业务记录主要存储在本地设备中；手机号验证码登录所需的手机号、验证码校验请求、登录状态及必要的服务端响应信息会发送至开发者配置的账号接口，并由阿里云号码认证服务处理短信验证码发送与校验。\n\n'
    ...
    LegalSectionContent(
      title: '6. 数据存储与安全',
      body:
          '当前版本中的主要业务数据保存在你的设备本地。手机号验证码登录所需的登录凭证会保存在本机，用于维持登录状态。我们会在应用能力范围内采取合理措施，尽量降低数据被意外丢失、误操作或未经授权访问的风险。\n\n'
    ...
    LegalSectionContent(
      ...
      body:
          ...
          '由于当前版本尚未提供开发者侧业务数据云同步，开发者通常无法为你恢复仅保存在本地设备中的账本数据。',
    ),
    LegalSectionContent(
      title: '8. 儿童与未成年人保护',
      body:
    ...
    LegalSectionContent(
      title: '9. 未来功能更新说明',
      body:
          '当前版本中，开发者不会通过本应用收集并上传用户在应用内录入的业务数据，但手机号验证码登录会按本政策说明处理手机号及登录校验所需信息。\n\n'
          '如未来版本引入以下能力，包括但不限于：\n'
          '• 云端同步、备份与恢复；\n'
          '• 行为分析工具；\n'
          '• 第三方服务接入；\n'
