...
//
// 本期口径（你确认）：
// - 日期：默认今天（但“口径真相”收口在 FormatUtils）
...
class SuggestService {
  ...
  //
  // ✅ 注意：
  // - “默认今天日期”的唯一真相在 FormatUtils.todayYmd()
  // - SuggestService 保留这些方法是为了兼容旧调用点（不破坏你现有代码）
  // - Step3 我们会逐页把调用点迁走，从而去掉页面对 SuggestService 的依赖
  // =====================================================================

  /// ✅ 兼容：默认今天 YYYYMMDD
  static String todayYmd({DateTime? now}) {
    return FormatUtils.todayYmd(now: now);
  }

  /// ✅ 兼容：可选格式列表（将来需要展示格式时再用）
  static List<String> todayFormats({DateTime? now}) {
    return FormatUtils.todayFormats(now: now);
  }

  /// ✅ 兼容：Autocomplete/默认值场景（返回单元素列表）
  static List<String> suggestTodayYmd({DateTime? now}) {
    return [FormatUtils.todayYmd(now: now)];
  }
