...
class SqfliteTimingRepository implements TimingRepository {
  ...
  Future<int> countByDeviceIdWithExecutor(
    DatabaseExecutor executor,
    int deviceId,
  ) async {
    final rows = await executor.rawQuery(
      'SELECT COUNT(*) AS count FROM $_table WHERE device_id = ?',
      [deviceId],
    );
    return (rows.single['count'] as num?)?.toInt() ?? 0;
  }
  ...
  Future<List<TimingRecord>> listByProjectId(String projectId) async {
    final db = await AppDatabase.database;
  ...
  Future<List<TimingRecord>> listByDeviceId(int deviceId) async {
    final db = await AppDatabase.database;
    return listByDeviceIdWithExecutor(db, deviceId);
  }
