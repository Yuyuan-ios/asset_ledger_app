...
IAP 订阅购买流程由生产服务端校验控制。早期版本若出包时忘了带
`dart-define`，App 会在 "立即升级" 页显示「暂不可购买 / 订阅购买服务暂不可用」
并禁用购买按钮；现在 release/product 构建内置了与 `production.json` 一致的生产
IAP base URL，避免手工 Xcode Archive 或本地 release 构建再次落回未接入状态。
...
| Key | 读取处 | 作用 | 默认 |
| --- | --- | --- | --- |
| `APPLE_IAP_VERIFICATION_BASE_URL` | `lib/core/config/subscription_config.dart` | 服务端票据校验后端 base URL；非空即开启购买流程并走服务端校验 | release/product：`https://api.yuyuan.net.cn/fleet-ledger`；debug/test：空 |
...
默认生产 URL 已在 release/product 构建配置，因此普通 release 构建会走服务端校验。
debug/test 构建仍保持空默认值，测试也可显式传空 `SubscriptionConfig` 覆盖
fail-closed 分支。
...
- `production.json`：正式方案。`APPLE_IAP_VERIFICATION_BASE_URL` 指向真实后端。
  当前值为 `https://api.yuyuan.net.cn/fleet-ledger`，CI 会断言它与代码默认值一致。
  使用前必须确认与实际部署一致，并在 Apple sandbox 中通过
  `/iap/apple/verify-purchase` 与 `/iap/apple/current-entitlement` 联调。
...
Xcode 里 Archive，dart-define 不会自动带上；IAP base URL 有代码默认值兜底，但
云备份等其它 define 仍可能漏配。请坚持用
