...
| `REVIEW_ACCESS_USER_IDS` | `lib/core/config/app_environment.dart` | 审核账号服务端用户 ID，逗号/空格分隔 | 空 |
...
不要把真实审核账号凭证提交到 git，也不要通过 `--dart-define` 编译进客户端。
审核账号必须先由真实登录系统认证；客户端只根据已认证用户的
`REVIEW_ACCESS_EMAILS` / `REVIEW_ACCESS_USER_IDS` whitelist 切换 sandbox access。
真实审核账号凭证只填写在 App Store Connect 和登录系统/后端安全配置中。
...
production build 若需要审核账号通道，只允许额外注入账号标识 whitelist：
...
不要在客户端、dart define、README 或测试 fixture 中写入审核账号凭证。若审核账号
需要固定验证码、登录凭证或免短信登录能力，必须在登录后端或管理后台配置，客户端不得
本地校验。
