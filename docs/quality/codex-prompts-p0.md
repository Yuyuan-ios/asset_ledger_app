...
5. **本地等价验收**：CI 无法在本机真触发，故每个 CI 切片必须 (a) 本地跑通对应命令，
   (b) 校验 workflow YAML。**本机 `python3` 无 PyYAML 且为 PEP 668 受管环境（禁直接 pip）**，
   用 macOS 自带的系统 ruby 校验（零安装）：
   `ruby -ryaml -e 'YAML.load_file(".github/workflows/flutter.yml"); puts "yaml OK"'`。
   装了 `actionlint` 则更佳。**不要为校验去 pip 安装 PyYAML / 改系统 Python。**
...
- **验证**（系统 ruby，零安装；注意 `on` 若未加引号会被 YAML 解析成布尔 `true` 键，故 `d["on"]||d[true]`）：
  `ruby -ryaml -e 'd=YAML.load_file(".github/workflows/flutter.yml"); on=d["on"]||d[true]; raise "dev missing" unless on["push"]["branches"].include?("dev") && on["pull_request"]["branches"].include?("dev"); raise "no concurrency" unless d["concurrency"]; puts "S1 OK"'`。
