...
- 当前未发现开发者自建登录、云同步、广告、分析或崩溃上报 SDK；订阅购买使用应用商店平台能力，生产构建如启用开发者侧订阅校验接口，需按实际传输内容复核隐私申报：
  `pubspec.yaml`
- 当前用到的系统/平台能力主要是：
  - `image_picker`
  - `in_app_review`
...
- 当前订阅购买由应用商店平台处理；如果生产构建启用了开发者侧订阅校验接口，需要按实际传输的交易校验信息重新评估是否仍能填 `No Data Collected`。
- 如果后续接入登录、云同步、错误上报、分析 SDK、广告 SDK 或任何新的远程 API，需要重新评估是否仍能填 `No Data Collected`。
...
- 订阅购买属于应用商店平台能力；如果生产构建新增开发者侧订阅校验、entitlement API 或任何会把交易校验信息传到开发者/第三方系统的流程，需要重新评估 Data safety。
- 已接入手机号登录和用户主动触发的云端备份后，Google Play Data safety / App Store 隐私披露需要按实际上线版本重新评估。
- 如果后续接入 Firebase / Crashlytics / Analytics、广告 SDK、自动多端同步、推送服务或其他远程接口，也必须再次重新填写 Data safety。
...
如果后续产品形态发生变化，尤其是新增开发者侧订阅校验、云同步、分析、崩溃上报或广告能力，这份结论需要重新评估。本文档用于提审准备与产品自查，不构成正式法律意见。
